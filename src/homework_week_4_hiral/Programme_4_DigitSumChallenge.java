...
public class Programme_4_DigitSumChallenge {
    public static int sumDigits(int number){
        if(number >=10) {
            int digit, sum = 0;
            while (number > 0) {
                //find the last digit ofthe given number
                digit = number % 10;
                //adds last digit to the variable
                sum = sum + digit;
                //removes the last digit from the number'number = number/10;
            }
            return sum;
        }else {
          //  return -1 for negative numbers and number
            return -1;
            }
        }

    public static void main(String[] args) {
        //scanner decdlaration fir reading input form consile
